...
func TestParseCityList(t *testing.T) {
	...
	contetns, err := ioutil.ReadFile("citylist_test_data.html")
	if err != nil {
		panic(err)
	}
	result := ParseCityList(contetns)
	...
	if len(result.Requests) != resultSize {
		t.Errorf("result should have %d"+"resuests; but had %d", resultSize, len(result.Requests))
	}
